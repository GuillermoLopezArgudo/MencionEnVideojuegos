...
public class GameManagerScript : MonoBehaviour
{
    ...
    public void ClickOnCard(string Nombre, int index)
    {
        ...
        else
        {
            ...
            if (CardUp == Nombre)
            {
                Destroy(Carta);
                Debug.Log("Pareja");
                Destroy(Cartas[index-1]);
                Destroy(Cartas[CardUpIndex-1]);
            ...
            else
            {
                ...
                StartCoroutine(Descanso(index, CardUpIndex));
    ...
    public void restart()
    {
        ...
        puntuacion = 0;
        Cartas.Clear();
        Carta.GetComponent<CardScript>().bandera = true;
        Start();
    }
        ...
        void Start()
    {
        ...
        for (var y = 0; y < 2; y++)
        {
            for (var x = -2; x < 3; x++)
            {
                ...
                newCarta.GetComponent<CardScript>().index = i;
                i++;
                Cartas.Add(newCarta);
                Baraja.RemoveAt(id);
                ID.RemoveAt(id);
    ...
    IEnumerator Descanso(int index, int CardUpIndex)
    {
        yield return new WaitForSeconds(2);
        Cartas[index - 1].GetComponent<CardScript>().Toogle();
        Cartas[CardUpIndex - 1].GetComponent<CardScript>().Toogle();
    }
